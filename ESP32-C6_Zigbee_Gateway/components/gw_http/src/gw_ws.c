...
static bool ws_encode_event(const gw_event_t *e, cbor_wr_t *w)
{
    ...
    enum {DATA_AUTOM_FIRED, DATA_AUTOM_RESULT, DATA_DEVICE_EVENT, DATA_DEVICE_STATE} data_kind;
    ...
    if (data_kind == DATA_DEVICE_EVENT) {
        ...
        if (!cbor_wr_text(w, "event") || !cbor_wr_text(w, "command")) return false;
...
esp_err_t gw_ws_register(httpd_handle_t server)
{
    ...
    if (xTaskCreate(ws_event_task_fn, "ws_events", 3072, NULL, GW_WS_EVENT_TASK_PRIO, &s_event_task) != pdPASS) {
