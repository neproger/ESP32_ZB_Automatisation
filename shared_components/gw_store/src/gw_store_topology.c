...
static void rebuild_endpoint_owner_index(void)
{
    ...
    for (size_t i = 0; i < GW_DEVICE_MAX_DEVICES * GW_DEVICE_MAX_ENDPOINTS; ++i) {
    ...
    for (size_t i = 0; i < micro_db_table_count(&s_endpoint_table); ++i) {
        ...
        gw_store_endpoint_key_t key = {.uid = record.uid, .endpoint = record.endpoint};
        uint32_t slot = 0;
...
static bool collect_endpoint_key_cb(const void *record, void *user_ctx)
{
    ...
    gw_store_endpoint_key_t *dst = &list->keys[list->count++];
...
{
    esp_err_t err = micro_db_table_init(&s_device_table, &GW_STORE_SCHEMA_TOPOLOGY_DEVICE);
    if (err != ESP_OK) {
        return err;
    }

    err = micro_db_table_init(&s_endpoint_table, &GW_STORE_SCHEMA_TOPOLOGY_ENDPOINT);
    if (err != ESP_OK) {
        (void)micro_db_table_deinit(&s_device_table);
        return err;
    }

    s_endpoint_owner_cap = next_owner_capacity(GW_DEVICE_MAX_DEVICES * GW_DEVICE_MAX_ENDPOINTS);
    s_endpoint_owners = (endpoint_owner_bucket_t *)calloc(s_endpoint_owner_cap, sizeof(*s_endpoint_owners));
    s_endpoint_next_slot = (uint32_t *)calloc(GW_DEVICE_MAX_DEVICES * GW_DEVICE_MAX_ENDPOINTS, sizeof(uint32_t));
    if (!s_endpoint_owners || !s_endpoint_next_slot) {
                                   ...
                                   bool *out_removed)
{
    ...
    gw_store_hook_remove_state_for_endpoint(key);
