...
# Zigbee Light Switch (End Device)

This project builds a Zigbee HA On/Off Switch **End Device** for ESP32-H2. It joins an existing Zigbee network (for example Home Assistant via ZHA or Zigbee2MQTT) and sends On/Off Toggle commands.
...
* One development board with ESP32-H2 SoC (this project)
* A USB cable for power supply and programming
* A Zigbee coordinator connected to your Zigbee network (for example, a USB dongle for Home Assistant)
...
Before flashing, it is recommended to erase NVRAM to remove any previous Zigbee network info using `idf.py -p PORT erase-flash`.

## Pairing (Home Assistant)

1. Put your coordinator into pairing mode:
   - ZHA: Settings → Devices & services → Zigbee Home Automation → Add device
   - Zigbee2MQTT: Permit join
2. Flash and reset the ESP32-H2. On first boot (factory-new), it starts network steering and should join automatically.
...
I (621) ESP_ZB_ON_OFF_SWITCH: Start network steering (join)
...
## Button Function
  ...
  * Toggling the button (BOOT) sends an On/Off Toggle Zigbee command. You can bind it to a light (or use it in automations) from your coordinator software.
