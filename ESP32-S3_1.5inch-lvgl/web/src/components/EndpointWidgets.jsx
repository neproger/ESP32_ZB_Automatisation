...
export default function EndpointWidgets({
  ...
  const sensorTemp = (() => {
    const epNum = Number(endpoint?.endpoint ?? 0)
    return state.temperature_c ?? getSensorValue(sensors, epNum, 0x0402, 0x0000)
  })()
  const sensorHum = (() => {
    const epNum = Number(endpoint?.endpoint ?? 0)
    return state.humidity_pct ?? getSensorValue(sensors, epNum, 0x0405, 0x0000)
  })()
