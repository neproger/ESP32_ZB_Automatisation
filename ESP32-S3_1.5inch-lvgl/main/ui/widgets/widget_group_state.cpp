...
bool widget_group_state_read(size_t group_index, size_t item_index, WidgetGroupState *out_state)
{
    ...
    *out_state = WidgetGroupState{};
    ...
    if (gw_group_store_get_by_index(group_index, &group) != ESP_OK) {
        return false;
    }
    ...
    const size_t item_count = gw_group_store_get_group_item_count(out_state->group.id);
    ...
    if (gw_group_store_get_group_item_by_index(out_state->group.id, resolved_item_index, &item) != ESP_OK) {
        return false;
    }
